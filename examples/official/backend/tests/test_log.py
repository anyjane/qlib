...
@pytest.mark.asyncio
async def test_list_logs_empty(log_dir):
    """测试列出日志文件 - 空目录"""
    api_client = APITestClient(client=pytest.fixture.__wrapped__(lambda: None))
...
@pytest.mark.asyncio
async def test_list_logs_with_files(log_dir):
    """测试列出日志文件 - 有文件"""
    # 创建测试日志文件
    log_files = [
        log_dir / "app.log",
...
@pytest.mark.asyncio
async def test_list_logs_sorted_by_modified(log_dir):
    """测试列出日志文件 - 按修改时间排序"""
    # 创建多个日志文件
    import time
    log_files = []
...
@pytest.mark.asyncio
async def test_download_log_file(log_dir):
    """测试下载单个日志文件"""
    # 创建测试日志文件
...
@pytest.mark.asyncio
async def test_download_log_not_found(log_dir):
    """测试下载单个日志文件 - 文件不存在"""
    api_client = APITestClient(client=pytest.fixture.__wrapped__(lambda: None))
...
@pytest.mark.asyncio
async def test_download_log_directory_not_exists():
    """测试下载日志文件 - 日志目录不存在"""
    # 临时移除日志目录
    import tempfile
    import shutil
...
@pytest.mark.asyncio
async def test_download_logs_batch(log_dir):
    """测试批量下载日志文件"""
    # 创建测试日志文件
    for i in range(3):
        log_file = log_dir / f"test_{i}.log"
...
@pytest.mark.asyncio
async def test_download_logs_batch_all(log_dir):
    """测试批量下载所有日志文件"""
    # 创建测试日志文件
    filenames = []
    for i in range(3):
...
@pytest.mark.asyncio
async def test_download_logs_batch_empty(log_dir):
    """测试批量下载日志 - 空列表"""
    api_client = APITestClient(client=pytest.fixture.__wrapped__(lambda: None))
    response = await api_client.post(
        "/api/logs/download/batch",
...
@pytest.mark.asyncio
async def test_download_logs_batch_non_existent_file(log_dir):
    """测试批量下载日志 - 包含不存在的文件"""
    # 创建一个测试日志文件
...
@pytest.mark.asyncio
async def test_download_large_log_file(log_dir):
    """测试下载大日志文件"""
    # 创建一个大日志文件（1MB）
...
@pytest.mark.asyncio
async def test_download_batch_many_files(log_dir):
    """测试批量下载大量文件"""
    # 创建 50 个日志文件
    filenames = []
    for i in range(50):
...
@pytest.mark.asyncio
async def test_list_logs_with_special_characters(log_dir):
    """测试列出包含特殊字符的日志文件名"""
    # 创建包含特殊字符的日志文件
    special_filenames = [
...
@pytest.mark.asyncio
async def test_download_log_path_traversal(log_dir):
    """测试下载日志文件 - 路径遍历攻击"""
    # 创建测试日志文件
    (log_dir / "test.log").write_text("content")
...
@pytest.mark.asyncio
async def test_download_batch_path_traversal(log_dir):
    """测试批量下载日志 - 路径遍历攻击"""
    (log_dir / "test.log").write_text("content")
